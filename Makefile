...
# Signing identity, team, and style are not passed here; swift-mk drives them
# through an XCODE_XCCONFIG_FILE override (from CODE_SIGN_IDENTITY / DEVELOPMENT_TEAM
# above) that wins over the project's per-target ad-hoc default for the app, the
# agent, and the helper in one build.
