...
final class CurveLearner: ObservableObject {
  ...
  private func persistRun(curve: [CurvePoint]) {
    ...
    do {
      ...
      log.error("learn.dir.failed path=\(dir.path, privacy: .public) error=\(error.localizedDescription, privacy: .public)")
      return
    }
  ...
  private func fitCurve() -> [CurvePoint] {
    ...
    for temp in filledTemps {
      ...
      let prev = known.last(where: { $0 < temp })
      let next = known.first(where: { $0 > temp })
      switch (prev, next) {
  ...
  private func probeTick(fanIndex: UInt, commandedRPM: Float) {
    guard let sensorState else { return }
    let target = sensorState.fans.first(where: { $0.id == fanIndex })
    if let fan = target {
