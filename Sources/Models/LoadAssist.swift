...
enum LoadAssistStore {
  ...
  static func migrateLegacyIfNeeded(defaults: UserDefaults) {
    ...
    for kind in LoadAssistKind.allCases {
      ...
      if defaults.object(forKey: kind.curvePointsKey) == nil {
        savePoints(defaultPoints(threshold: threshold, floorPercent: floorPercent), kind: kind, defaults: defaults)
      }
    }
  ...
  static func normalizedPoints(_ points: [CurvePoint]) -> [CurvePoint] {
    ...
    if normalized.first?.temperature ?? 0 > loadRange.lowerBound {
      normalized.insert(CurvePoint(temperature: loadRange.lowerBound, fanPercent: normalized.first?.fanPercent ?? 0), at: 0)
    }
    if normalized.last?.temperature ?? 100 < loadRange.upperBound {
      normalized.append(CurvePoint(temperature: loadRange.upperBound, fanPercent: normalized.last?.fanPercent ?? 0))
    }

    var previousTemp = loadRange.lowerBound
    var previousPercent = 0.0
    for index in normalized.indices {
      let maxTemp = index == normalized.indices.last ? loadRange.upperBound : loadRange.upperBound - Double(normalized.count - index - 1)
